...
-- RLS Policies (RELAXED FOR DEVELOPMENT)
...
CREATE POLICY "Users can view orders for their customers" ON sales_hub_orders
  FOR SELECT USING (
    EXISTS (
      ...
      AND (assigned_sales_rep = auth.uid() OR auth.jwt()->>'role' IN ('admin', 'manager', 'user'))
    ) OR auth.jwt()->>'role' IN ('admin', 'manager', 'user')
  );
...
CREATE POLICY "Users can create orders" ON sales_hub_orders
  FOR INSERT WITH CHECK (created_by = auth.uid() OR auth.jwt()->>'role' IN ('admin', 'manager', 'user'));
...
CREATE POLICY "Users can update orders they created" ON sales_hub_orders
  FOR UPDATE USING (created_by = auth.uid() OR auth.jwt()->>'role' IN ('admin', 'manager', 'user'));
