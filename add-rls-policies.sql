...
-- Basic RLS policies
DROP POLICY IF EXISTS "Allow authenticated access to system_metrics" ON system_metrics;
CREATE POLICY "Allow authenticated access to system_metrics" ON system_metrics FOR ALL USING (auth.role() = 'authenticated');
DROP POLICY IF EXISTS "Allow authenticated access to marketing_campaigns" ON marketing_campaigns;
...
DROP POLICY IF EXISTS "Allow authenticated access to marketing_strategies" ON marketing_strategies;
...
DROP POLICY IF EXISTS "Allow authenticated access to marketing_kpis" ON marketing_kpis;
